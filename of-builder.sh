...
if [[ ! -d "$THISSCRIPTPATH/../$GITREPOLIN" ]]; then
	...
	[[ "$USER" != "$GITLINUXOOWNER" ]] && LSTSH=$(sudo -u $GITKERNELOWNER -H git --rebase -C "$THISSCRIPTPATH/../$GITREPOLIN" stash) || LSTSH=$(git -C "$THISSCRIPTPATH/../$GITREPOLIN" stash)
	[[ "$USER" != "$GITLINUXOOWNER" ]] && LPULL=$(sudo -u $GITKERNELOWNER -H git --rebase -C "$THISSCRIPTPATH/../$GITREPOLIN" pull) || LPULL=$(git -C "$THISSCRIPTPATH/../$GITREPOLIN" pull)
