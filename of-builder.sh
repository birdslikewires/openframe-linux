...
## of-builder.sh v1.51 (7th September 2025)
...
KCONFIGFILE=`ls $THISSCRIPTPATH/../$GITREPOKER/configs | grep "$KLATESTMAJVER"."$KLATESTMIDVER"op.config`
if [[ $KCONFIGFILE == "" ]]; then
	echo
...
if [[ "$KBUILDIT" == 0 ]]; then
	...
	if [ $KLATESTMIDVER -gt 5 ] && [ $KLATESTMIDVER -gt 2 ]; then
...
if [[ "$IBUILDIT" == 0 ]]; then
	...
	$THISSCRIPTPATH/of-imgcreate.sh "$(echo ${ICODENAME,,} | head -c 3)" ext2 1 uni 46 "${IDISTNAME,,} ${ICODENAME,,}" "$THISSCRIPTPATH/../$GITREPOLIN/overlay-${IDISTNAME,,}-${ICODENAME,,}" "$KDLPATH" "$IDOWNLURL"
